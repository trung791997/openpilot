...
## D-055 — BLoTv2's t_follow pads saturate rather than vanish, and the crawl hold latches on necessity, not on the exact floor

**Decided 2026-09-17. Implemented in `selfdrive/controls/lib/blotv2.py`. Ported from upstream
BLoTv3 (`SpysyWeeb/Spysypilot`, branch `combo-blotv3`, `necessity_supervisor.py` at `7aed876`,
design note `docs/BLoTv3.md` §3). Unit evidence only — no replay, no road. BLoTv3 itself is
marked "in progress, not field validated" by its own author.**

Our supervisor is a port of upstream BLoTv2 (b38e933 closed five gaps against it). Upstream has
...
**What this is not.** Unit tests only, on a supervisor that is gated behind the `BlotV2` toggle.
Nothing here has been replayed or driven. Both changes widen following distance or keep the jerk
