  ...
  computed: {
    ...
    searchResults() {
      ...
      return this.sections
        .map((s) => {
          const descendants = new Set(PERSONALITY_PROFILES_ENABLED ? ["CustomPersonalities"] : [])
          let size
          ...
          return { ...s, matches: s.params.filter(p => this.isProfileCard(p) ? s.params.some(child => descendants.has(child.key) && this.matchesFilter(child)) : !descendants.has(p.key) && this.matchesFilter(p)) }
        })
          ...
          <template v-for="section in searchResults" :key="section.slug">
            <GalaxySection :title="tr(section.name, section.name) + ' (' + section.matches.length + ')'" :icon="section.icon || 'bi-search'" :default-open="false">
              ...
              <template v-for="p in section.matches" :key="p.key">
                <PersonalityProfiles v-if="isProfileCard(p)" :manage-open="!!expanded[p.key]" @manage="toggleManage(p.key)" @change="onParamChange" />
