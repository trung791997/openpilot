...
export const api = {
  ...
  getLatTuneWorkspace() { return request("/api/lat_tune/workspace", { cache: "no-store" }) },
  getLatTuneStatus() { return request("/api/lat_tune/status", { cache: "no-store" }) },
  getLatTuneTrial(trialId) { return request(`/api/lat_tune/trial/${encodeURIComponent(trialId)}`, { cache: "no-store" }) },
  latTuneAnalyze(routes) { return request("/api/lat_tune/analyze", { method: "POST", data: { routes } }) },
  latTuneStopAnalyze() { return request("/api/lat_tune/analyze/stop", { method: "POST" }) },
  latTuneApplyTrial(trialId, force = false) { return request(`/api/lat_tune/trial/${encodeURIComponent(trialId)}/apply`, { method: "POST", data: { force } }) },
  latTuneRevertTrial(trialId) { return request(`/api/lat_tune/trial/${encodeURIComponent(trialId)}/revert`, { method: "POST" }) },
  latTuneDeleteTrial(trialId) { return request(`/api/lat_tune/trial/${encodeURIComponent(trialId)}`, { method: "DELETE" }) },
