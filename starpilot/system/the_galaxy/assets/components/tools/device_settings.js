...
// This branch's planner still reads the legacy CustomPersonalities follow/jerk settings and does
// not consume LongitudinalPersonalityProfiles, so the profile panel stays off and the legacy
// subtree renders instead. Flip only together with a planner port that reads the profiles.
const PERSONALITY_PROFILES_ENABLED = false
...
// This branch's planner still reads the legacy acceleration/following settings and does not
// consume LongitudinalPersonalityProfiles, so keep the legacy controls and hide the profile
// editors until a planner port wires them in.
...
function getSectionsWithSlug() {
  return state.layout
    ...
    .map(section => {
      const personalityLegacySubtreeKeys = PERSONALITY_PROFILES_ENABLED ? collectDescendantKeys(section.params, "CustomPersonalities") : new Set()
      return {
        ...section,
...
function getSettingLockReason(param) {
  if (PERSONALITY_PROFILES_ENABLED && param?.key === "CustomPersonalities" && state.personalityMigrationRequired) {
    ...
    <div class="ds-row ${isNumeric ? "ds-row-numeric" : ""} ${isText ? "ds-row-text-input" : ""} ${isChild}">
      <div class="ds-row-info">
        <div class="ds-row-text">
          ...
          ` : ""}
          ${() => p.key !== LONGITUDINAL_MODE_KEY && p.is_parent_toggle && ((PERSONALITY_PROFILES_ENABLED && p.key === "CustomPersonalities") || isParamEnabledForChildren(p)) ? html`
            <button type="button" class="ds-manage-btn"
              aria-controls="${PERSONALITY_PROFILES_ENABLED && p.key === "CustomPersonalities" ? "personality-profiles-panel" : `ds-${p.key}-children`}"
              aria-expanded="${() => state.expanded[p.key] ? "true" : "false"}"
              @click="${() => toggleManage(p.key)}">
              ${state.expanded[p.key] ? "Close" : "Manage"}
              <i class="bi bi-chevron-${state.expanded[p.key] ? "up" : "down"}" aria-hidden="true"></i>
...
function renderSettingTree(paramsList, parentKey = null) {
  ...
  for (const param of directChildren) {
    ...
    if (PERSONALITY_PROFILES_ENABLED && param.key === "CustomPersonalities") {
      if (state.expanded[param.key]) rendered.push(renderPersonalityProfilesPanel())
      continue
    }
